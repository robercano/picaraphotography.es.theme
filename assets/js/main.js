...
var App = {
	...
	image_copyright: function() {
		...
		jQuery(document).on('contextmenu', '#categorizr .nav-holder, #pattern-full, #gallery, .isotope-holder a, #rail a, #journal-list .item img, .slider .item img', function(e) {
		...
		categorizr: function(direction) {
			...
			switch(direction) {
				case 'in':{
					...
					jQuery('#categorizr .slider .item:first').addClass('visible');
					jQuery('#categorizr .slider .item:first').addClass('zindex');
					
					function rotateSmooth(index) {
					}
					
					setTimeout(function() { rotateCover() }, 10000);
					function rotateCover() {
						var item = jQuery('#categorizr .slider .item.zindex');
						var index = jQuery('#categorizr .slider .item').index(item);
						var length = jQuery('#categorizr .slider .item').length;
